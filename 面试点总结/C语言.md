...
- <font color="#ff0000">忘记释放内存</font>
	- 使用`delete`或`free`<font color="#00b0f0">释放内存</font>
...
# **sizeof和strlen的区别**
- `strlen()`是一个<font color="#ff0000">字符串函数</font>，只能计算<font color="#ff0000">字符串的长度</font>，遇到'\0'就会结束,不会把'\0'也一并计算进去
- `sizeof()`是一个<font color="#ff0000">运算符</font>, 能够计算出数据类型、变量、常量的<font color="#ff0000">大小</font> (计算指针时返回的是指针本身的大小)
...
- **条件预处理指令**
	- 使用预处理指令 (如`#ifdef`、`#ifndef`、`#if`、`#elif`、`#else`和`#endif`). <font color="#ff0000">通过定义或未定义宏</font>, 可以设置条件并根据条件选择性地编译代码
- **平台检测**
...
- **系统与架构级优化**
	...
	- 缓存友好编程
		- 让数据访问在内存中尽可能连续
			- 遍历多维数组时，按行遍历（内存连续）而非按列遍历
		- 结构体对齐与填充
			- 调整结构体成员顺序，减少因内存对齐造成的“空洞”
	- 循环优化
		- 减少循环内计算量, 将不随循环变量变化的计算移动到循环外部
		- 避免在循环内部调用昂贵的函数
		- 当循环次数较少时, 尽量展开循环
- **代码级优化**
	- 内联函数
...
## **堆内碎块**
## **定义**
- <font color="#00b0f0">堆内存</font>中被<font color="#ff0000">分割成许多小块</font>，虽然总空闲内存足够，但无法满足较大内存分配请求的情况
## **类型**
- **外部碎片**：空闲内存被分散成多个小块
- **内部碎片**：分配的内存块比实际需要的大
## **预防**
...
- 加入`extern`修饰的是变量的<font color="#00b0f0">声明</font>, 说明此变量将在文件以外或在文件后面部分定义
...
```
## **前缀命名法**
在变量名前添加前缀以表示其作用域、类型或特殊含义
## **作用域前缀**
|前缀|含义|示例|说明|
|---|---|---|---|
|`g_`|全局变量|`g_total_users`|明确标识全局作用域|
|`s_`|静态变量|`s_instance_count`|文件作用域的静态变量|
|`m_`|模块私有|`m_internal_state`|模块内使用的变量|
|`p` / `ptr_`|指针变量|`p_node`, `ptr_next`|明确表示指针类型|

## **类型前缀 (简化版匈牙利命名法)**
|前缀|类型|示例|
|---|---|---|
|`i_`|整型|`i_length`, `i_count`|
|`f_`|浮点型|`f_ratio`, `f_percentage`|
|`d_`|双精度|`d_coordinate`|
|`b_`|布尔型|`b_is_valid`, `b_initialized`|
|`sz_`|字符串|`sz_name`, `sz_path`|
|`arr_`|数组|`arr_students`, `arr_buffer`|
|`ch_`|字符|`ch_delimiter`|

## **语义前缀**
|前缀|含义|示例|
|---|---|---|
|`tmp_`|临时变量|`tmp_buffer`, `tmp_result`|
|`old_`|旧值|`old_position`, `old_size`|
|`new_`|新值|`new_config`, `new_settings`|
|`curr_`|当前值|`curr_index`, `curr_state`|
|`prev_`|前一个值|`prev_node`, `prev_value`|
|`next_`|下一个值|`next_item`, `next_page`|
...
| 后缀     | 含义   | 示例                     |
| ------ | ---- | ---------------------- |
| `_t`   | 类型定义 | `size_t`, `time_t`     |
| `_ptr` | 指针类型 | `node_ptr`, `file_ptr` |
| `_arr` | 数组类型 | `int_arr`, `char_arr`  |
## **语义后缀**
| 后缀                 | 含义     | 示例                           |
| ------------------ | ------ | ---------------------------- |
| `_count`           | 数量     | `user_count`, `error_count`  |
| `_size`            | 大小     | `buffer_size`, `file_size`   |
| `_max` / `_min`    | 最大/最小值 | `temp_max`, `level_min`      |
| `_idx` / `_index`  | 索引     | `array_idx`, `current_index` |
| `_len` / `_length` | 长度     | `string_len`, `list_length`  |
| `_str`             | 字符串    | `name_str`, `path_str`       |
| `_msg`             | 消息     | `error_msg`, `status_msg`    |
## **状态后缀**
|后缀|含义|示例|
|---|---|---|
|`_flag`|标志位|`debug_flag`, `enable_flag`|
|`_status`|状态|`connection_status`, `operation_status`|
|`_result`|结果|`calculation_result`, `search_result`|
|`_error`|错误|`parse_error`, `io_error`|
# **预编译的定义以及使用场景**
<font color="#00b0f0">预编译</font>是指在实际编译之前对源代码进行的<font color="#ff0000">预处理</font>阶段
使用场景 :
- 跨平台使用条件编译
- 宏定义常量 (更直观更易修改)
- 头文件保护 (防止头文件被重复包含和编译)
- 代码配置和特性开关 (根据编译选项启用不同功能)

# **结构体大小**
- 每个成员相对于结构体起始<font color="#00b0f0">地址的偏移量</font>必须是<font color="#ff0000">该成员类型大小的整数倍</font>
- <font color="#00b0f0">结构体总大小</font>必须是最<font color="#ff0000">宽成员大小的整数倍</font>
...
## **宏函数**
## **优点**
- **执行效率高**：没有函数调用的开销（压栈、跳转、返回）
- **通用性强**：不关心参数类型，可实现泛型编程
- **编译时展开**：在预处理阶段就完成替换
## **缺点**
- **容易出错**：参数可能被多次求值
- **调试困难**：编译器看到的是展开后的代码
- **代码膨胀**：每次调用都会展开，增加代码大小
- **无类型检查**：编译时不会检查参数类型
## **普通函数**
## **优点**
- **类型安全**：编译器会检查参数和返回值类型
